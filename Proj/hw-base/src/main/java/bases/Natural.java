...
public class Natural {
  ...
  public static int leadingDigit(int[] digits) {
    ...
    // Inv: D[i+1], ..., D[n-1] are all zeros
    while (digits[i] == 0 && i != 0) {
      i--;
    }

    // At this point in the code, we know that D[i+1], ..., D[n-1] are all zeros,
    // and D[i] != 0 or i = 0
  ...
  public Natural toBase(int base) {
    ...
    while (i != 0) {
      ...
      j += 1;
      r = r.times(b);
      r = r.plus(new Natural(base, digits[i]));
    }
  ...
  public String toString() {
    ...
    // This implies the postcondition below, since buf = ch(D[n-1]), ..., ch(D[0])
  ...
  public Natural plus(Natural other) {
    ...
    // Add A and B with respect to each digit until i - 1
    ...
    while (i != other.digits.length) {
      newDigits[i] = digits[i] + other.digits[i];
      i++;
    }

    // A[0] < b and B[0] < b => D[0] < 2b, this can be extended to all the digits
    // First we have i = other.digits.length, so D[0] = A[0]+B[0], D[1] = A[1]+B[1],
    // ..., D[i-1] = A[i-1]+B[i-1] satisfies.
    // Extend B's digits s.t. all the digits that is longer than its length are zeros
    // Inv: D[0] = A[0]+B[0], D[1] = A[1]+B[1], ..., D[i-1] = A[i-1]+B[i-1],
    ...
    // D[0] = A[0]+B[0], D[1] = A[1]+B[1], ..., D[n-1] = A[n-1]+B[n-1],
    //      where B[k] = other.digits[k] if k < other.digits.length and
    //            B[k] = 0               otherwise
    // this.value = D[0] + D[1] b + ... + D[n-1] b^{n-1}
